#ifndef CRC_H
#define CRC_H



#define GETDATA_RET_OK 0
#define GETDATA_RET_NODATA 1
#define GETDATA_RET_SIZEMISMATCH 2
#define GETDATA_RET_NRMISMATCH 3
#define GETDATA_RET_CMDMISMATCH 4
#define GETDATA_RET_DATSIZEMISMATCH 5
#define GETDATA_RET_APIVERSIONMISMATCH 6
#define GETDATA_RET_UNKNOWN -1
#define GETDATA_RET_NODATA -2


static const uint8_t crc_table[] = {
    
};

uint8_t
crc8(uint8_t *p, uint8_t len)
{
        uint16_t i;
        uint16_t crc = 0x0;

        while (len--) {
                i = (crc ^ *p++) & 0xFF;
                crc = (crc_table[i] ^ (crc << 8)) & 0xFF;
        }

        return crc & 0xFF;
}

void sendData(printerCmd *cmd, uint8_t *p,uint8_t size) {
  cmd->datasize = size;
  
  char buf[ sizeof(printerCmd)+MAX_DATA_SIZE];
  memset(&buf,0, sizeof(printerCmd)+MAX_DATA_SIZE);
  
  memcpy(&buf[0],(uint8_t *) cmd, sizeof(printerCmd));
  memcpy(&buf[sizeof(printerCmd)],p, size);
  
#ifdef CB_DEBUG 
  Serial.print("Buffer: ");
  for (int i =0; i<sizeof(printerCmd)+MAX_DATA_SIZE; i++){
    Serial.print(buf[i],HEX);
    Serial.print(" ");
  }
  Serial.println("END");
#endif
  Wire.write(buf, sizeof(printerCmd)+MAX_DATA_SIZE);

}

int getData(printerCmd &cmd, uint8_t * buffer, uint8_t size,uint8_t& bytesrecv ) {
  bool retval;
  int a,cCount;
  char c[sizeof(printerCmd)+size];

  
  a= Wire.available();
  if (a==0){
    return GETDATA_RET_NODATA;
  }


  cCount=0;
  while (cCount<a){
    c[cCount] = Wire.read();
    cCount++;
  }
  
  /*if (cCount != sizeof(printerCmd) + size) {
    
    return GETDATA_RET_SIZEMISMATCH;
  }
  else {  */

    printerCmd retCmd;
    
    memcpy(&retCmd,&c[0],sizeof(printerCmd));
    
    byte cmdNr = retCmd.nr & 15;

    
    if (retCmd.cmd != cmd.cmd) 
      return GETDATA_RET_CMDMISMATCH;
    else if ((retCmd.nr>>4) != API_VERSION) 
      return GETDATA_RET_APIVERSIONMISMATCH;  
    //else if (retCmd.datasize != size) 
    //  return GETDATA_RET_DATSIZEMISMATCH;  
    else {
      if (size>0) {
        memcpy(buffer+bytesrecv,((uint8_t *) &c[0])+sizeof(printerCmd),size);
        bytesrecv+=retCmd.datasize;
      }
      return GETDATA_RET_OK;
    }
  //}
  
  return GETDATA_RET_UNKNOWN;
}

int sendCommand(int slave, int command, uint8_t * ptrInput, uint8_t sizeInput, uint8_t * ptrOutput, uint8_t sizeOutput) {
  
  bool morepackets=true;
  int r = GETDATA_RET_OK;
  
  delay(10);
  Wire.beginTransmission(slave);
  printerCmd cmd;
  
  cmd.nr = 1 | (API_VERSION<<4);
  cmd.cmd = command;
  

    
  sendData(&cmd,ptrInput,sizeInput);
  Wire.endTransmission();

  
  if (sizeOutput >0){
    r = GETDATA_RET_NODATA;
    uint8_t bytesrecv=0;
    while (morepackets) {
      delay(10);
      
      Wire.requestFrom(slave, sizeof(printerCmd) + MAX_DATA_SIZE);
      
      
      if (Wire.available()){
        r= getData(cmd,ptrOutput,sizeOutput,bytesrecv);
      }
      
      morepackets =(cmd.nr & CB_FLAG_MORE_PACKETS )== CB_FLAG_MORE_PACKETS;
    }
  }
  return r;
}

#endif //CRC_H

